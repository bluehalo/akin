...
const UserSimilaritySchemaWrite = new Schema({
	users: { type: [String] },
	similarity: { type: Number }
}, {collection: 'recommendations.userSimilaritiesWrite'} );
UserSimilaritySchemaWrite.index({ users: 1 });
...
const UserRecommendationSchemaWrite = new Schema({
	user: { type: String },
	recommendations: { type: [ ItemWeightModel.schema ] }
}, {collection: 'recommendations.userRecommendationsWrite'} );
UserRecommendationSchemaWrite.index({ user: 1 });
...
mongoose.model('UserSimilarityWrite', UserSimilaritySchemaWrite);
mongoose.model('UserRecommendationWrite', UserRecommendationSchemaWrite);
