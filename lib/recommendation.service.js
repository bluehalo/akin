...
const transformUserSimilaritiesForUser = (userId, userSimilarities) => {
    ...
    return Promise.all(_.map(userSimilarities, (userSimilarity) => {
        return {
...
const calculateUsersRecommendations = ( allUserIds ) => {
    return Promise.all(_.map(allUserIds, calculateAndSaveUserRecommendations));
};
 ...
 * for the user again
 */
const markRecommendationDNR = (userId, itemId, itemType) => {
