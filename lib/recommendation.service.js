...
const _ = require('lodash'),
	...
	UserRecommendationWrite = ModelService.model.UserRecommendationWrite,
 ...
 */
const calculateAndSaveUserRecommendations = ( userId, i ) => {
    var threshold = 0.1;
    ...
    return ModelService.getAllSimilaritiesForUser(userId, threshold)
        ...
        .then( (recommendationsToSave) => {
            ...
            return UserRecommendationWrite.insertMany([recommendationsToSave])
                    .then(() => {
...
const calculateUsersRecommendations = ( allUserIds ) => {
    console.log('calculateUsersRecommendations', allUserIds.length)
...
const recalculateUserRecommendations = () => {
    ...
    console.timeEnd('recalculateUserSimilarities')
    console.time('recalculateUserRecommendations')
    return ModelService.dropUserRecommendationsWrite()
        .then( ModelService.getAllUserIdsWithActivity )
        .then( calculateUsersRecommendations )
        .catch( (err) => {
            console.timeEnd('recalculateUserRecommendations')
