 ...
 */
let getSamples = (collection = [], sampleSize = 1) => {

    // clone the original input array since we'll be removing elements from it
    let elements = _.clone(collection);

    let actualSamples = _.min([_.size(collection), sampleSize]);
 ...
 */
let getItemIdToWeightMap = (userItemWeights) => {
 ...
 */
let filterRecommendations = (dnr, itemIdToWeightMap, rec) => {
...
let addScoreToItems = (scoreMap, items) => {
    return _.map(items, (r) => {
 ...
 */
let sampleRecommendationsForUser = (userId, numberOfSamples) => {
    ...
    return Promise.join(recommendationPromise, itemWeightsPromise, doNotRecommendPromise, (userRecommendationsObj, userItemWeights, dnr) => {
...
module.exports.sampleRecommendationsForUser = sampleRecommendationsForUser;
