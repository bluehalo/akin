 ...
 * @param {object} recommentation the recommendation that may be filtered out
 */
const filterRecommendations = (dnr, itemToWeightMap, recommentation) => {
    // If the item is in the DNR list, ignore it
    if(!_.isEmpty(dnr)) {
        var dnrEntry = _.find(dnr.doNotRecommend, { item: recommentation.item });
    ...
    return recommentation.weight > 0.5 || itemToWeightMap[recommentation.item] <= 2;
};
